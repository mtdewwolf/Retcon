...
async fn apply_patch(repo: &Path, patch: &str, prefix: &[&str]) -> Result<(), GitError> {
        ...
        tokio::select! {
            result = stdout.read(&mut stdout_chunk), if !stdout_done => {
                ...
                if read == 0 {
                    ...
                    if stdout_bytes.len().saturating_add(read) > MAX_GIT_OUTPUT_BYTES {
                        overflow = true;
                        let _ = child.kill().await;
                    } else {
                        stdout_bytes.extend_from_slice(&stdout_chunk[..read]);
            ...
            result = stderr.read(&mut stderr_chunk), if !stderr_done => {
                ...
                if read == 0 {
                    ...
                    if stderr_bytes.len().saturating_add(read) > MAX_GIT_OUTPUT_BYTES {
                        overflow = true;
                        let _ = child.kill().await;
                    } else {
                        stderr_bytes.extend_from_slice(&stderr_chunk[..read]);
    ...
    if overflow {
        return Err(GitError {
            command: args.join(" "),
            exit_code: status.code(),
            stderr: format!("git apply output exceeded {MAX_GIT_OUTPUT_BYTES} bytes"),
        });
    }
