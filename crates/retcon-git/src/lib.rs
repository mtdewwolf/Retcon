...
impl std::fmt::Display for GitError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "git {} failed ({:?}): {}", self.command, self.exit_code, self.stderr)
    }
}
...
pub async fn status(repo: &Path) -> Result<RepoStatus, GitError> {
    ...
    let entries = porcelain
        .lines()
        .filter(|l| l.len() > 3)
        .map(|l| StatusEntry { code: l[..2].to_owned(), path: l[3..].to_owned() })
        .collect();
...
pub async fn worktree_list(repo: &Path) -> Result<Vec<Worktree>, GitError> {
    ...
    for line in raw.lines() {
        if let Some(path) = line.strip_prefix("worktree ") {
            ...
            current = Some(Worktree { path: path.to_owned(), branch: None });
...
pub async fn worktree_add(repo: &Path, path: &str, branch: &str) -> Result<(), GitError> {
    run_git(repo, &["worktree", "add", "-b", branch, path]).await.map(|_| ())
}
...
pub async fn worktree_remove(repo: &Path, path: &str) -> Result<(), GitError> {
    run_git(repo, &["worktree", "remove", path]).await.map(|_| ())
}
